...
Имеем две компоненты связности:
![[exam_cs10.png]]
...
$$g(1) + g(3) = 2 \Longrightarrow g(3) = 1$$
$$g(3) + g(8) = 3 \Longrightarrow g(8) = 2.$$
...
В чём заключается задача о максимальном потоке? Представим себе связный взвешенный и направленный граф как сеть труб, где рёбра — трубы, а вершины — точки разъединения. Каждое ребро имеет вес, равный пропускной способности трубы.
Есть две вершины: $s$ — источник и $t$ — сток. Каков максимальный поток от источника $s$ к стоку $t$ на этом графе?
